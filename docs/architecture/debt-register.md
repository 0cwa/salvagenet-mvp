...
| P0 | F01 canonical profile/manifest hardening is not yet merged | The primary split source of truth is removed, but focused review found the legacy bare-file fallback initially accepted non-Podroid IDs. The corrected head requires full revalidation before it can become repository truth. | Require active manifests for Ubuntu/AAVMF and all non-Podroid artifacts, pass the complete workflow on the exact hardened documentation head, then merge PR #7 and move this item to resolved debt. |
