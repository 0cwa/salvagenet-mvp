...
| P0 | F01 canonical profile/manifest foundation is validated but not yet merged | The split source of truth is removed and compatibility migration is verified on the branch, but `main` does not become authoritative until PR #7 merges its exact tested documentation head. | Implementation head `71a04acedd11221fbefe2c0fa43984141ec11ed4` passed Actions run `30543765626`. Pass the final documentation workflow, merge PR #7, record the merge SHA, and move this item to resolved debt. |
