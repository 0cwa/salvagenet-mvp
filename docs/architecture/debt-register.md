...
| P0 | F01 canonical profile/manifest foundation is reviewed and validated but not yet merged | The split source of truth is removed and all actionable PR #7 findings are addressed, but `main` is not authoritative until the final documentation head passes and PR #7 merges. | Reviewed implementation head `b0dae0581c9d72ae0f7481f7b602573931fbc3a2` passed Actions run `30548116488`. Pass the final documentation workflow, resolve review threads, merge only the exact tested head, record the merge SHA, and move this item to resolved debt. |
