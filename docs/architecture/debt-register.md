...
| Split production profile and artifact-manifest sources of truth | F01 head `a343b3dc283d22fe49bbef3caefb6e05f446f4a8` passed Actions run `30533830617`: Android loads strict packaged canonical JSON, the APK verifies exact profile/guest-init bytes, and publication/listing/cleanup/runtime use one manifest adapter. The result remains non-physical and becomes repository truth when PR #7 merges. |
