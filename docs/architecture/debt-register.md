...
| P0 | Agent-run physical testing lacked local exclusion, authorization, and candidate provenance | Concurrent agents, destructive operations, or dirty worktrees could otherwise change the same phone or mislabel diagnostic APKs as promotable evidence | Merge the serial lease, expiring scenario/action authorization, diagnostic/candidate modes, exact-input evidence, and promotion rejection before unattended local device runs. |
...
| P1 | Guest class, concrete image identity, source provider, and distro bootstrap are not yet independent contracts | Imported artifacts remain deployable only where packaged profiles name them, and future user choices could otherwise become an invalid-combination matrix | Follow the guest-class/image-source ADR after H02A and physical Ubuntu evidence; separate image binding before adding a distro matrix or Android-native OCI pulling. |
...
| MVP+ | USB/AOA networking | Adds a Linux daemon, custom framing, TAP/NAT, permissions, reconnect, MTU, and a hardware matrix | Keep blocked until every base gate is PASS; retain the plan in docs rather than a placeholder implementation root. |
...
| Truthful Ubuntu writable-layer contract | PR #10 merged at `d13d4d721d2a6f3d37acfcbaea7bca7e50498b4e`; the profile, schema, typed model, resolver, and tests name the implemented copy-once mutable qcow2 mode `copied-writable` and no longer claim a backing-file overlay. |
...
| Complete remote software/package reproduction | GitHub Actions run `30510377089` passed static/contracts, controller, JVM, Android/lint, guest qualification, packaged APK, signature, alignment, and candidate upload for the validated H01 head. |
