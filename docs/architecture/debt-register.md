...
| P0 | Production profile and artifact consumption have split sources of truth | Checked-in JSON is documented as canonical, while Android constructs profiles in Kotlin and runtime/listing paths parse artifact manifests independently; lab success can diverge from APK behavior | F01: load validated packaged JSON in production and use one strict versioned manifest contract before H02/H03 or final physical guest evidence. |
