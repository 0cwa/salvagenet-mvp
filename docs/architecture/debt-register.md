...
|---|---|---|---|
| P0 | Physical-device evidence is missing | Ten product gates remain `BLOCKED-HARDWARE`; software, roadmap, website, and host-QEMU confidence cannot prove Android/OEM, VpnService, APK-native QEMU, reboot, or controller-offline behavior | Complete the bounded WEB04 coordination foundation, reactivate H02A, then execute the existing HIL vertical slice when hardware is available. Diagnostic device runs may happen earlier, but final evidence must bind one exact candidate commit/APK. |
| P0 | Canonical Ubuntu guest boot has not been qualified end to end on host QEMU | F01 guarantees shared profile and artifact semantics, but the real Ubuntu/AAVMF/NoCloud/QMP/SSH/restart path has not yet produced reviewed bounded evidence | H02A is queued for reactivation immediately after the sole active WEB04 foundation. Prove boot independently of guest mesh, inspect secret residue, and record exact profile/artifact identities before considering H02B. |
| P1 | Repository-wide GPLv2 release position is not documented | Podroid is GPLv2 and the APK links SalvageNet modules; an APK release without a declared compatible license, notices, corresponding source, and modification records creates avoidable compliance ambiguity | Before distributing a public release APK, complete EA-00 licensing review, add the repository-level license/notice policy, and verify the published source exactly corresponds to the binary. |
