...
|---|---|---|---|
| P0 | Physical-device evidence is missing | Ten product gates remain `BLOCKED-HARDWARE`; software confidence cannot prove Android/OEM, VpnService, APK-native QEMU, reboot, or controller-offline behavior | After the current canonical-profile foundation and focused preflight review, execute the existing HIL vertical slice. Diagnostic device runs may happen earlier, but final evidence must bind one exact candidate commit/APK. |
...
| P1 | `node-shell` owns too many production responsibilities | Composition, enrollment, artifact policy, API wiring, TLS, guest bootstrap, profile storage, and recovery are difficult to review independently | Extract only after a phase demonstrates a review/debugging blocker. F01 may introduce a narrow profile/manifest boundary but must not become a broad package split. |
...
|---|---|
| Split Podroid import/wiring folklore | The pinned subtree, external integration hook, ordered patch series, offline metadata check, and network reconstruction check are owned by `tools/vendor/podroid.py`. |
