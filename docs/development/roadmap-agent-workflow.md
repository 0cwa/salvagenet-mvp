...
5. update the issue with the merge and evidence result;
6. close the issue only when its implementation outcome is complete;
7. leave acceptance gates unchanged unless their own required evidence passed;
8. run the next phase-start review instead of automatically activating the next queued issue.
