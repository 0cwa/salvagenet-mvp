...
|---|---|---|---|---|---|
| F01 | 2026-07-30 | `docs/research/experiments/F01.md`, PR #7, Actions run `30549498423` | Repository, Android build, and GitHub CI | PASS | Canonical packaged profile JSON and one strict artifact-manifest adapter are merged; activate only deterministic Ubuntu guest-boot qualification next. Unreleased migration compatibility is not required. |
| F00 | 2026-07-30 | `docs/research/experiments/F00.md`, PR #6, Actions run `30524125036` | Repository and GitHub CI | PASS | Phase 0 passed: H01/H04 were archived as merged, H02/H03 were queued for review, F01 became the sole active foundation, and phase-boundary acceptance verification was enforced. |
...
| E06 | 2026-07-29 | `evidence/gates/B04.json`, `B10.json`, F01, active H02 packet | Host/profile tests; no Android QEMU boot | SOFTWARE-PASS/GUEST-PREFLIGHT-OPEN/HARDWARE-OPEN | Packaged JSON is now the production source. H02 must prove deterministic host-QEMU UEFI/NoCloud/key-only SSH before repeated Android UEFI boot and guest access close the live question. |
| E07 | 2026-07-29 | `evidence/gates/B06.json`, `B11.json`, `B20.json` | Bootstrap/recovery/redaction tests | SOFTWARE-PASS/HARDWARE-OPEN | Current one-use redemption design is retained; H02 and the physical guest slice must inspect actual guest persistence and logs. |
