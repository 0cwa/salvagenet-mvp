...
retry until LocalAPI reports `Running`, then is atomically removed. Logout must
succeed before local identity/configuration state is cleared.
...
limits. Status address output is capped at 16 entries. Diagnostic failures are
classified without including enrollment material.
