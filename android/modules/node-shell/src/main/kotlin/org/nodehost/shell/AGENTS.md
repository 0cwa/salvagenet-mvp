...
- Every non-Podroid artifact requires an active manifest. Do not add compatibility migration code without an actual deployed state and an explicit migration task.
- Bare-file plus `.sha256` resolution is restricted to `podroid-kernel`, `podroid-initramfs`, and `podroid-alpine-squashfs`.
...
- Production profile resolution consumes packaged validated JSON; a complete Kotlin mirror is prohibited.
