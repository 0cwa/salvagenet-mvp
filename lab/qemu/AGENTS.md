...
- A test-only NoCloud user-data layer may supply an ephemeral SSH public key and mesh-independent readiness marker. It must not create `/var/lib/nodehost/bootstrap.env` or introduce auth keys, bootstrap tokens, callback capabilities, Headscale, or Tailscale.
