...
while IFS= read -r -d '' file; do bash -n "$file"; done < <(find "$root" -type f -name '*.sh' -not -path '*/android/podroid/*' -not -path '*/.git/*' -not -path '*/.worktrees/*' -print0)
...
if grep -RIn --include='*.sh' --exclude-dir=.git --exclude-dir=.worktrees --exclude-dir=podroid \
...
if command -v shellcheck >/dev/null 2>&1; then
  mapfile -d '' files < <(find "$root" -type f -name '*.sh' -not -path '*/android/podroid/*' -not -path '*/.git/*' -not -path '*/.worktrees/*' -print0)
