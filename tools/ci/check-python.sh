...
python3 -m compileall -q "$root/tools" "$root/controller/mvp-cli" "$root/tests/tools" "$root/tests/hil" "$root/lab/headscale/tests"
...
PYTHONPATH="$root" python3 -m unittest discover -s "$root/tests/hil" -p 'test_*.py'
